# encoding: utf-8
# @author: LQK
# @desc: 此版本已弃用, 目标网站招聘列表页已改为 Ajax 请求, 添加 瑞数验证, 登陆验证
# @version: 0.1
# @Time: 2020/06/24
...
def parseJob(url, **kwargs):
    ...
    job_list = response.xpath(
        "//div[contains(@class, 'main_current_items')]//div[contains(@class,'details_container')]")
    ...
    for jobs in job_list:
        ...
        item['职位名称'] = jobs.xpath(".//span[@class='post']/a/@title").get()
        item['职位url'] = jobs.xpath(".//span[@class='post']/a/@href").get()
        item['薪资'] = jobs.xpath(".//span[@class='salary']/text()").get()
        item['工作地点'] = jobs.xpath(".//span[@class='address']/text()").get()
        item['发布时间'] = '20' + jobs.xpath(".//span[@class='release_time']/text()").get()

        item['企业名称'] = jobs.xpath(".//span[@class='company_name']/a/@title").get()
        item['企业url'] = jobs.xpath(".//span[@class='company_name']/a/@href").get()
        item['工作经验'] = \
        jobs.xpath("./div[@class='introduction_container']//span[contains(text(), '经验')]/text()").get().split('：')[1]
        item['学历'] = \
        jobs.xpath("./div[@class='introduction_container']//span[contains(text(), '学历')]/text()").get().split('：')[1]
        item['公司规模'] = \
        jobs.xpath("./div[@class='introduction_container']//span[contains(text(), '规模')]/text()").get().split('：')[1]
        item['公司性质'] = \
        jobs.xpath("./div[@class='introduction_container']//span[contains(text(), '性质')]/text()").get().split('：')[1]
