...
headers = {
        ...
        "x-csrf-token-creis-city": "xX0wMJQXja9C4VFE8EDydh0O",  # TODO
...
def get_params_id(cityId):
    ...
    id = [i['id'] for i in [code['propertyTypeList'] for code in code_json if code['dealTableSource'] == 'month'][0] if i['name']=='普通住宅']
...
def down_load_json(bs, pages, beginDate, endDate, city_code, data_all, city):
    ...
    payload = {"cityId": city_code, "cityName": city, "districtIdList": [],
               ...
               "colType": "指标", "spreadType": "汇总", "orderType": "desc", "isTrusted": True, "dealTableSource": "month",
               "beginDate": beginDate, "endDate": endDate, "pageNum": 1, "pageSize": 100}
    print(f"{payload}")
...
def run(bs):
    ...
    for city in city_id_df['cityName'].tolist():
        if city not in ['广州', '惠州', '茂名', '梅州', '深圳', '佛山', '清远',
            '东莞', '江门', '阳江', '湛江', '肇庆', '中山', '汕头', '珠海', '韶关']: continue
        print()
...
if __name__ == '__main__':
    ...
    # 合并
    path_name = '广东省_全市项目成交_月.xlsx'
    writer = pd.ExcelWriter(path_name)
    d = []
    d1 = []
    for i in os.listdir('全市_data/月'):
        print(i)
        city = i.split('.')[0]
        path = os.path.join('全市_data/月', i)
        df1 = pd.read_excel(path, sheet_name='2016-2020')
        d.append(df1)

    df1 = pd.concat(d)
    df1.to_excel(writer, sheet_name='2016-2020', index=False)
    writer.save()
    writer.close()
