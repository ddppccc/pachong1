...
def get_parseInfo(city,dist,liltename, url):        # 解析 函数
    ...
    while True:
        ...
        if has_spider.find_one({'标题qwfsdhjgh':url}):
            ...
            house_div = html.xpath("//div[@class='zu-itemmod']")    #  房子信息==============
            try:
            ...
            has_spider.insert_one({'标题qwfsdhjgh':url})       #   插入

            next_page_url = html.xpath('string(.//div[@class="multi-page"]/a[@class="aNxt"]/@href)')  # 判断下一页是否存在
            if next_page_url:
