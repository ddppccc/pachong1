...
pagebase = pymongo.MongoClient('mongodb://{}:{}@{}:{}/'.format(
            ...
            MONGODB_CONFIG['host'],
            MONGODB_CONFIG['port']),
            retryWrites="false")['大众点评']['西安_列表页链接_202108']
info_base = pymongo.MongoClient('mongodb://{}:{}@{}:{}/'.format(
            MONGODB_CONFIG['user'],
            MONGODB_CONFIG['password'],
            MONGODB_CONFIG['host'],
            MONGODB_CONFIG['port']),
            retryWrites="false")['大众点评']['西安_数据_202108']
hasurl = pymongo.MongoClient('mongodb://{}:{}@{}:{}/'.format(
            MONGODB_CONFIG['user'],
            MONGODB_CONFIG['password'],
            MONGODB_CONFIG['host'],
            MONGODB_CONFIG['port']),
            retryWrites="false")['大众点评']['东莞_去重_202107']
...
if __name__ == '__main__':
    ...
    while pagebase.find_one({'status':-1}):           #状态0未抓取  -1正在抓取  1已抓取
        page=pagebase.find_one({'status': -1})
